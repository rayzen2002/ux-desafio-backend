...
export class UsersService {
  ...
  async CreateUser(data: {name: string; email: string, password: string, phone?: string; cpf: string }){
    const existingUser = await this.usersRepository.findByEmail(data.email)
    if (existingUser?.length) throw new ConflictException('Email já cadastrado')
